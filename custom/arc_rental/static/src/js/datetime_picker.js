...
const PRECISION_LEVELS = new Map()
    .set("days", {
        ...
        getItems: (
            date,
            { additionalMonth, maxDate, minDate, showWeekNumbers, isDateValid, dayCellClass, disable_days }
        ) => {
            ...
            return startDates.map((date, i) => {
                ...
                for (let w = 0; w < 6; w++) {
                    ...
                    for (let d = 0; d < 7; d++) {
                        ...
                        // manage BY SS
                        // disable_days
                        if(disable_days && disable_days.includes(dayItem.id)) {
                            dayItem.isValid = false;
                            dayItem.extraClass += ' unavailabledates'
                        }
...
export class DateTimePicker extends Component {
    ...
    onWillRender() {
        ...
        const getterParams = {
            ...
            disable_days: this.state.disable_days,
        };
    ...
    adjustFocus(values, focusedDateIndex) {
        ...
        if(this.state.precision == "days"){
            this.rpc('/arc_rental/disable_days', {
                start_month: this.clamp(dateToFocus.startOf("month")).startOf("month").month,
                start_year: this.clamp(dateToFocus.startOf("month")).startOf("month").year,
                rental_time: window.rental_time,
                product_id: $(".oe_website_sale input[name='product_id']").val() || false,
                additionalMonth: this.additionalMonth,
            }).then(function (response) {
                self.manage_dates(response);
            })
        }
        this.state.focusDate = this.clamp(dateToFocus.startOf("month"));
    }

    manage_dates(response) {
        this.state.disable_days = response;
        this.state.focusDate = this.state.focusDate;
    }
    /**
    ...
    next(ev) {
        ...
        if(this.state.precision == "days"){
            this.rpc('/arc_rental/disable_days', {
                start_month: this.clamp(this.state.focusDate.plus(step)).startOf("month").month,
                start_year: this.clamp(this.state.focusDate.plus(step)).startOf("month").year,
                rental_time: window.rental_time,
                product_id: $(".oe_website_sale input[name='product_id']").val() || false,
                additionalMonth: this.additionalMonth,
            }).then(function (response) {
    ...
    previous(ev) {
        ...
        if(this.state.precision == "days"){
            this.rpc('/arc_rental/disable_days', {
                start_month: this.clamp(this.state.focusDate.minus(step)).startOf("month").month,
                start_year: this.clamp(this.state.focusDate.minus(step)).startOf("month").year,
                rental_time: window.rental_time,
                product_id: $(".oe_website_sale input[name='product_id']").val() || false,
                additionalMonth: this.additionalMonth,
            }).then(function (response) {
    ...
    zoomIn(date) {
        ...
        if (index in this.allowedPrecisionLevels) {
            // SSS Code
            let self = this;
            if(this.allowedPrecisionLevels[index] == "days"){
                this.rpc('/arc_rental/disable_days', {
                    start_month: this.clamp(date).startOf("month").month,
                    start_year: this.clamp(date).startOf("month").year,
                    rental_time: window.rental_time,
                    product_id: $(".oe_website_sale input[name='product_id']").val() || false,
                    additionalMonth: this.additionalMonth,
                }).then(function (response) {
